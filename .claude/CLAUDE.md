...
- 【AI生成フローの自動テスト化】ウィザード（`wizardDecomposeForms`等）・テーブルスキーマ生成（`tblAiGenerateSchema`）・バリデーションルール生成（`validAiGenerateRules`）は対応済み（2026-09-18実装）。`extRtGenDoc`・`textToYamlGenerate`・`manualRetryAiFix`・`formDesignTextToYamlGenerate`・`yamlAiGenerate`も対応済み（2026-09-20〜21、下記参照）。残り（`formDesignAiGenerate`）のみDOM読み書きがロジック本体に密結合しており未着手（ウィジェット全削除＋再描画という破壊的操作を伴うため最後に回している）
