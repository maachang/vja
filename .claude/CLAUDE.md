...
- 【未解決・重要】`yamlAiGenerate`実行時、`$("yaml-ta")`（YAMLエディタ本文）が生成の瞬間に空になり、依頼内容が丸ごとAIに渡らず文脈の無いコードが生成される不具合（2026-09-21発覚、原因のメカニズム未特定）
  - **確認済みの事実のみ記載**: (1) この問題は過去数ヶ月発生していなかった。(2) 2026-09-19時点では`/home/maachang/project/work/wja-test/table-test.vjaproj`の`tableViewButton`の`Click`イベントで正常なコードが生成されていた（実ログでuserLen=2174を確認）。(3) 2026-09-21に同じ操作を行うと、userLenが正確に`1632`という一貫した値になり、生成されるJSコードも`SELECT * FROM horse_info`のみの最小限のものになる不具合が複数回（同一の数値で）再現している。(4) 実際にAIへ送信されたuserPrompt本文をログ出力して確認したところ、「[The Following YAML]」ブロック（依頼内容そのもの）が完全に欠落しており、ウィジェット一覧も自分自身（`tableViewButton`）以外がすべて欠落していた＝生成の瞬間、`$("yaml-ta")`が空だったことを直接確認済み。(5) しかし生成直前に表示される確認ダイアログ（「JavaScriptタブに既存のコードがあります」）は正常に表示されており、これは`yamlAiGenerate`冒頭のYAML空チェック（後述の対応済み内容）を通過した後に出るものなので、**チェック時点では`yaml-ta`は空でなかった**ことも確認済み。(6) 確認ダイアログ（`vja.app.showConfirm`）は`#dialog-root`という別コンテナを使っており、YAMLエディタ本体（`#modal-root`）のDOMを破壊しないこともコード上確認済み
  - **未解決**: チェック時点（確認ダイアログ表示前）から実際の生成時点までの間に、何が`$("yaml-ta")`を空にしているのか、具体的なメカニズムはまだ特定できていない
  - **これまでに検証し否定できた仮説**: 今回のセッションで行ったリファクタ（`yamlAiGenerate`のDOM分離等、複数コミット）が原因ではない。2026-09-19時点の旧コード（コミット`01399ce`）でも、実際に`.vjaproj`を読み込んで同一条件（`yaml-ta`が空の状態）を再現すると同じ`userLen=1632`になることを実測で確認済み。つまりこの不具合は今回のリファクタ以前から存在する既存の潜在バグであり、今回のリファクタで新たに発生したものではない
  - **対応済みの部分的な安全策**: `yamlAiGenerate`冒頭（確認ダイアログより前）に、`$("yaml-ta")`が空の場合は生成自体を中止しトースト通知する処理を追加済み（コミット`60daa62`）。ただし上記(5)の通り、この対策は「確認ダイアログの後・生成の直前」に空になるケースは検知できない
  - **調査用に一時追加した未コミットのデバッグログ**: `vja-modal.js`の`runAiGenerate()`内、`[AI] request:`ログの直後に、実際に送信される`userPrompt`全文をログ出力する行を追加している（未コミット、原因特定後は削除予定）
  - **次のアクション（原因のメカニズムが具体的に説明できるまで、コード修正は行わないこと）**: 確認ダイアログ表示中〜生成呼び出し直前の間で`$("yaml-ta")`に影響し得る処理（`saveYamlData()`の中身、`showLoadingModal()`がmodal-rootに与える影響等）を1つずつ実際に読み、再現条件を絞り込む必要がある
