...
- 【AI生成フローの自動テスト化】ウィザード（`wizardDecomposeForms`等）・テーブルスキーマ生成（`tblAiGenerateSchema`）・バリデーションルール生成（`validAiGenerateRules`）は対応済み（2026-09-18実装）。`extRtGenDoc`も対応済み（2026-09-20、下記参照）。残り（`yamlAiGenerate`/`formDesignAiGenerate`/`formDesignTextToYamlGenerate`/`textToYamlGenerate`/`manualRetryAiFix`）はDOM読み書きがロジック本体に密結合しており、テストハンドラ追加にはDOM操作とロジックを分離するリファクタが前提になるため未着手。リスクの低い順（`textToYamlGenerate`→`manualRetryAiFix`→`formDesignTextToYamlGenerate`→`yamlAiGenerate`→`formDesignAiGenerate`）に1つずつ対応する方針（2026-09-20合意。`formDesignAiGenerate`はウィジェット全削除＋再描画という破壊的操作を伴うため最後に回す）
