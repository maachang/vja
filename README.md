...
| 環境 | モデル | 生成時間 | コード精度 |
|------|--------|----------|----------|
| M1 Mac 16GB | gemma4-e4b（mlx-lm） | キャッシュ有: 約40秒 | 実用＋コメント |
...
アクション:
  - horse_info テーブル内容をsqlで以下の条件で検索し、その結果を tableView に表示させる:
    - 前提条件: 「ローディング処理」を表示
    - 検索条件あり: searchText に文字が存在する場合は searchSelect で選択されたカラム名で「部分一致」検索
    - 検索条件なし: 条件なしで検索
...
try {
  await vja.ui.loading(true, "検索中...");

  // 入力値の取得
  searchValue = vja.widget.get("searchText");
  columnName = vja.widget.get("searchSelect");

  // 基本となるSQL文
  sql = "SELECT * FROM horse_info";
  params = [];

  // 検索条件の構築
  if (searchValue && searchValue !== "") {
    // searchSelectで選択されたカラム名を使用して部分一致検索を行う
    // カラム名はプレースホルダが使えないため、ホワイトリスト形式での検証が必要だが、
    // 今回は仕様に基づき動的に組み立てる（※実運用ではカラム名のバリデーションを推奨）
    sql += ` WHERE ${columnName} LIKE ?`;
    params.push(`%${searchValue}%`);
  }

  // データ取得
  rows = await vja.db.query(sql, params);

  // テーブルへの表示
  vja.widget.set("tableView", rows);

} catch (error) {
  console.error(error);
  vja.notify.toast("データの取得中にエラーが発生しました。");
} finally {
  await vja.ui.loading(false);
}
~~~

**完全ローカル = データがクラウドに出ない。月額コスト 0 円。**

また、多くの開発者が利用している、M1 Mac などAppleシリコンであれば `mlx-lm` を利用することで「Appleシリコンに搭載されている GPU や NPU を利用して、効率よく高速なローカルLLMを利用できる」ので、この環境なら正に投資０円で vja を利用する事ができます。
