...
なお、この「生成 → 検証 → エラーをフィードバックして再生成」というループ自体は、Claude Code のような汎用コーディングエージェントが持つ自己修正ループと本質的には同じ考え方だと捉えています。ただし規模と仕組みは大きく異なります。

- **汎用コーディングエージェント（Claude Code / OpenCode 等）**: ファイル全体・会話履歴・多数のツール定義といった大きなコンテキストを毎回送り、テスト実行や型チェックなど実際の実行結果を読みながら、必要な回数だけ自律的に修正を繰り返すオープンエンドなループ
- **VJA**: 「1イベント＝数十〜数百行」という狭いタスクに徹底して切り出し、決まった短い出力スキーマのみを生成させ、固定回数（1回）の自動リトライと軽量な静的チェック・モック実行で完結させる設計

汎用エージェントをローカル LLM に接続すると、大きなコンテキストの prefill・複雑なツール呼び出しの往復コストが重く、非力なローカル推論エンジンでは著しく速度が落ちることがあります。VJA が採用している「タスクを狭く切り出し、短い入出力に絞る」という設計は、まさにローカル LLM が得意とする使い方そのものであり、同時にクラウド API 利用時のトークン単価コストの低さにも直結しています。つまり、**同じ「タスクを狭く切り出す」という設計判断が、ローカル LLM の速度問題とクラウド API のコスト問題を同時に解決している**、という構造だと考えています。汎用エージェントと VJA は、そもそも解こうとしている問題のスコープが異なる、と捉えるのが実情に近いでしょう。
