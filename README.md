...
OpenAI の最新軽量・高効率モデル（**GPT-6 Luna** や **GPT-4o-mini** など）を利用すれば、**ほぼタダ同然の超低コスト**で VJA をフル活用できます。
...
- 1リクエストの消費トークン数が極めて少なく、1回の生成コストは **0.1円前後** レベル（下記「GPT-6 Lunaでのコスト試算」参照）
- 画面1つ、アプリ1つを完成させるまで何度も生成・修正を繰り返しても **合計で数円** 程度
...
#### GPT-6 Lunaでのコスト試算（実測プロンプトサイズベース）

GPT-6 Lunaの料金（入力: $0.10 / 100万トークン、出力: $0.50 / 100万トークン）を前提に、vja実際のプロンプト（`src/mainview/prompts/*.md`）から実測した文字数をもとに試算した結果です。

代表例として「イベントJS自動生成」（1イベント単位でのコード生成、vjaのAI生成機能の中核）のシステムプロンプト＋典型的なユーザープロンプト（対象ウィジェット数件・参照テーブル1件程度の小〜中規模イベント）を実測すると：

| 対象 | システムプロンプト実測 | ユーザープロンプト実測(典型例) | 合計文字数 |
|------|------------------------|-------------------------------|-----------|
| フロントイベント(JS) | 約10,057字 | 約1,200字 | 約11,300字 |
| バックエンドイベント(TS・アプリイベント) | 約6,566字 | 約1,200字 | 約7,800字 |

プロンプトは英語主体のため、概算で1トークン≈4文字として換算すると：

| 対象 | 入力トークン概算 | 入力コスト | 出力トークン概算(生成コード規模による) | 出力コスト | 1回あたり概算合計 |
|------|-------------------|-----------|------------------------------------------|-----------|---------------------|
| フロントイベント | 約2,800トークン | 約$0.00028 | 約500〜1,500トークン | 約$0.00025〜$0.00075 | **約$0.0005〜$0.001（約0.1円前後）** |
| バックエンドイベント | 約1,950トークン | 約$0.0002 | 約500〜1,500トークン | 約$0.00025〜$0.00075 | **約$0.0004〜$0.001（約0.1円前後）** |

→ **1イベントのコード生成あたり、概ね0.1円前後**。中規模アプリ（画面5枚・イベント20件程度）を画面デザイン生成も含め一通り作成しても、合計で数円程度に収まる計算です。

※ 上記はあくまで実測プロンプトサイズと料金からの概算であり、実際のトークン数はモデル固有のトークナイザー・生成コードの長さによって変動します。正確なコストは利用しているAI事業者側の実際の課金明細でご確認ください。

### 動作環境・モデル比較

| 方式 | 環境 | 推奨モデル | 特徴・コスト |
|------|------|-----------|------------|
| **クラウド API** | あらゆる PC（軽量ノート等） | **GPT-6 Luna** / **GPT-4o-mini** | **超低コスト（数円/アプリ）**・超高速・高精度 |
...
### 🧙 プロジェクト新規作成ウィザード
- 「作りたいものを日本語で話すだけ」で、フォーム構成・DBテーブルの雛形を一括生成
- AIが1問ずつ質問を動的に生成する対話形式のヒアリング（選択肢ボタン・番号入力にも対応、いつでも「完了」で終了可）
- ヒアリング結果から必要なフォーム一覧・DBテーブル候補をAIが提案 → 確認モーダルで取捨選択 → 一括生成
- 生成後は他のAI雛形生成機能（画面デザイン・イベント処理・テーブル・検証）と組み合わせて仕上げる、プロジェクト立ち上げの最初の一歩を担う機能。詳細は[ウォークスルー](docs/sample-wizard-walkthrough.md)参照
...
> この構成でも、直近のWindowsアップデート以降は起動失敗やファイルオープン後のフリーズは発生しておらず、安定して動作しています（2026-09-25時点）。
...
### 「ウィジェット配置・命名」という一番地味な作業を、ローカル LLM がエージェント的に肩代わりする

画面デザインYAMLドラフト生成に「🖼 レイアウトイメージ選択」を組み合わせたことで、これまで人間が画面ごとに手作業で行っていた「ウィジェットをドラッグして並べ、1つ1つに名前を付ける」という、地味だが手数のかかる作業が、ほぼ丸ごとローカル LLM 任せにできるようになりました。

1. 「作りたい画面」を日本語で一言書く（例: 「ユーザ名とパスワードでログインする画面」）
2. AI が依頼文から入力項目・アクション（YAML）と、画面の大まかな構成（レイアウトイメージ）を同時に判断・生成する
3. 「🤖 画面反映」で、そのYAMLとレイアウトイメージを踏まえてローカル LLM がウィジェットの実座標を計算・配置する

人間が担うのは「日本語で一言書く」ことと「出来上がった画面の微調整」だけで、**座標計算・整列・命名といった機械的な作業はローカル LLM が代行する**という、まさに「ローカル LLM が開発作業そのものを支援するエージェント」的なワークフローになっています。クラウドの大規模コーディングエージェントのような重厚な仕組みではなく、**「1画面＝1回の小さな生成」という軽量なタスクに切り出しているからこそ、非力なローカル LLM でもエージェント的な支援が現実的に成立する**、という点は本プロジェクト全体の設計思想（「システムプロンプトは渡せば渡すほど良いわけではない」節を参照）とも一貫しています。
...
### YAMLは「発想と構造化記法をつなぐ翻訳レイヤー」である

画面デザイン・イベント処理・DBテーブル・バリデーション・そして後述するプロジェクト新規作成ウィザード。VJAのAI雛形生成機能に共通しているのは、**「AIにコードを書かせる」のではなく「発想（自然言語）を構造化記法（YAML/JSON）へ翻訳させ、その先の実際の処理・配置は既存の確定的なロジックに委ねる」**という設計です。

バリデーションルールやDBテーブルのカラム定義は、思いつくこと自体は一瞬でも、決まった記法に手で書き起こすのは地味に面倒な作業です。YAMLドラフト機能もイベント処理YAMLも、この「発想 → 構造化記法」という変換コストをAIに肩代わりさせている点で本質的に同じ仕組みです。この変換レイヤーの精度が上がるほど、YAMLの書き方を覚える前段のハードルが下がり、ノーコードに近い体感が強まります。プロジェクト新規作成ウィザードは、この変換パターンの適用範囲を「1機能」から「1プロジェクト全体」に広げたものと位置づけられます。

### 「ゼロから作る」より「たたき台を直す」方が総コストが低い

AIにアプリを一度で完成させるよう要求すると、画面レイアウト・入力項目・イベント処理という大きな一貫性を一発で保証させる必要があり、失敗確率も生成トークン量も跳ね上がります。VJAが採用している「まず雛形を作り、後から個別のYAMLドラフトで直す」設計は、この記事の「1イベント単位の細分化リクエストでトークン消費が極小」という強みを、コード生成だけでなくプロジェクトの立ち上げ段階にも適用したものです。大きな一撃生成ではなく、小さく確実な生成の積み重ねなので、失敗時のやり直しコストも低く抑えられます。

人間側の作業コストにも同じ非対称性があります。「白紙の状態から要件を思い出しながら組み立てる」のと「既にある雛形を見て『ここが違う』と直す」は、認知的な負荷が全く違います。後者の方が圧倒的に取り組みやすく、これはAIによる一次生成の精度が完璧でなくても、**「たたき台を出す→人が直す」という前提に立った設計自体が、総コストを引き下げる**という実感につながっています。
