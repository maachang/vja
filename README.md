...
vjaを身近な環境で試せるものとして、現在多くの開発者が利用している、M1 Mac などの「Appleシリコン搭載PC」がある。

ここでは `mlx-lm` を利用することで「Appleシリコンに搭載されている GPU や NPU を利用して、効率よく高速なローカルLLMを、PCが熱くならずに利用」できる。

そしてこの環境では、投資0円で `vja` を無理なく利用する事ができる。
