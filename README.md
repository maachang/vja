...
### MoE構成の非推論モデル（DeepSeek-Coder-V2-Lite）でも同様の傾向を確認

Apple Silicon Mac（16GBメモリ）や他環境で **DeepSeek-Coder-V2-Lite（4bit量子化）** を試したところ、Qwen2.5-Coder-7Bと同種の傾向（高速・実用的な精度）が確認できました。

- 総パラメータ16B / アクティブパラメータ2.4BのMoE（Mixture of Experts）構成
- **アクティブパラメータが2.4Bと小さいため、体感速度はQwen2.5-Coder-7B（稠密7B）とほぼ同程度**。MoEは総パラメータの大きさではなく、アクティブパラメータの小ささが速度を左右することが実感できる一例
- 総パラメータ16B相当の知識量があるためか、精度もそれなりに良好（ただし`const`を使ってしまう等、細かなルール違反は見られる。これは本READMEの「生成コードの自動検証（ガチャ対策）」で行コメントとして検出される）
- Qwen2.5系と同じく、推論（thinking）ブーム以前の世代のモデルであり、VJA環境でもかなり高速に実行される

アーキテクチャの異なる2系統のモデル（Qwen2.5＝稠密、DeepSeek-Coder-V2-Lite＝MoE）で同じ傾向（非推論・高速・実用的な精度）が確認できたことで、「VJAのタスク設計と非推論モデルの組み合わせが噛み合っている」という見立ての裏付けが強まった形です。16GBメモリ環境のようなメモリ制約下でも、MoEモデルという選択肢が有効に働くケースがある、という実例としても記録しておきます。
