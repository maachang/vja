...
- M1-MacでのAI生成コード(Qwen2.5-7B-coderで生成):
~~~javascript
...
vjaを身近な環境で試せるものとして、2020年末以降に発売された「Appleシリコン搭載Mac（M1以降）」がある。これらはすでに広く普及しており、追加投資不要でvjaを利用できる環境が整っている。

⇑の実行例では Appleシリコン専用の `mlx-lm` を利用しており、これは「Appleシリコンに搭載されている GPU や NPU を利用して、効率よく高速なローカルLLMを、PCが熱くならずに利用」することができる(キャッシュONの場合、約20秒でjavascriptを生成)。

そしてこのAppleシリコン環境だと、投資0円で `vja` を無理なく利用する事ができる。
