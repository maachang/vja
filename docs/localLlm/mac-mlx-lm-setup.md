...
以下の手順を対話式で自動化した `setup-mac-llm.sh` をプロジェクト直下に用意しています。

```sh
./setup-mac-llm.sh
```

- Homebrewがインストール済みであることが前提です（未導入の場合はエラーで案内して終了します）
- セットアップ先ディレクトリ・ポート番号・max-tokens・temperatureを質問形式で確認します（Enterでデフォルト値を採用可能）
- モデルは `Qwen2.5-Coder-7B-Instruct-4bit` 固定です
- 完了後、フォアグラウンド起動用（`start-llm.sh`）とバックグラウンド起動用（`start-llm-bg.sh`）の2つのスクリプトが指定ディレクトリに生成されます
- サーバーの自動起動は行いません（初回はモデルダウンロードで時間がかかるため、案内された起動コマンドを任意のタイミングで実行してください）

手動でセットアップしたい場合、または別モデルを使いたい場合は、以下の手順を参照してください。
