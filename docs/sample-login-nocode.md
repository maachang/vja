...
その後、「ユーザ登録画面への導線も欲しい」と思い、同じフォームに追加でボタンを1つ足す指示を出し、`btnRegUser`（表示テキスト「ユーザ登録」）を追加しました。
...
同じ名前のボタンでも、フォームが違えば独立して別々のJSが生成されます（ウィジェットを複製した場合は要注意 — 複製直後は内部的に元ウィジェットの生成済みJSを共有してしまう不具合が過去にあったため、複製後は必ずJSを再生成するか、生成済みの内容を確認してください）。
