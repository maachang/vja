...
- **await漏れ検出**: 本来`await`が必須のAPI（`vja.db.query`等）が、`await`無しで呼び出されていないかを確認します
...
3. 未知のAPI・禁止パターン・await漏れ・未知のウィジェット名は、該当行の末尾に `// ⚠ 未知のAPI: ...` のような指摘コメントが自動で挿入されます（構文エラー・モック実行エラーは行の特定精度が低いため、バナーでの通知のみです）
