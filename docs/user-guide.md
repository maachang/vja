...
- **禁止パターン検出**: `require()`の使用、ヘルパー関数の定義、`.then()`/`.catch()`の使用など、VJAのイベント処理として書いてはいけない構造が混ざっていないかを確認します

問題が検出された場合の流れは以下の通りです。

1. 検出内容（該当行・問題点）を添えて、AIに**1回だけ**自動で修正を依頼します
2. 修正後も問題が解消しない場合、生成自体は中断せず、コードはそのままJavaScriptタブに反映されます
3. 該当行の末尾に `// ⚠ 未知のAPI: ...` のような指摘コメントが自動で挿入されます
4. 画面上部に警告バナーが表示され、「🤖 もう一度AIに修正を依頼」（何度でも実行可）または「このまま閉じる」を選べます

> この検証は「構文的・構造的に明らかにおかしいか」を機械的にチェックするものであり、「意味的に正しく動くか」までは保証しません。検証を通過したコードでも、実際に動かして確認することをおすすめします。
