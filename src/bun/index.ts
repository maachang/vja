...
const buildEventsJs = (form: any, allForms: any[]): string => {
    ...
    lines.push('function _vjaErrDetail(label, key, e) {');
    lines.push('  try {');
    lines.push('    var decoded = decodeURIComponent(escape(atob(_vjaB64[key])));');
    lines.push('    var codeLines = decoded.split("\\n");');
    lines.push('    var errLine = (e && e.line) ? e.line - 2 : null; // AsyncFunctionが先頭に2行追加するためオフセット補正');
    lines.push('    var errCol = (e && e.column) ? e.column : null;');
    lines.push('    var start = errLine ? Math.max(0, errLine - 3) : 0;');
    lines.push('    var end = errLine ? Math.min(codeLines.length - 1, errLine + 1) : codeLines.length - 1;');
    lines.push('    var excerpt = codeLines.slice(start, end + 1).map(function(l, i) {');
    lines.push('      var lineNo = start + i + 1;');
    lines.push('      var marker = lineNo === errLine ? " >" : "  ";');
    lines.push('      return marker + " " + String(lineNo).padStart(3) + ": " + l;');
    lines.push('    }).join("\\n");');
    lines.push('    var location = errLine ? " (" + errLine + "行目" + (errCol ? " " + errCol + "列" : "") + ")" : "";');
    lines.push('    return label + " エラー詳細: " + (e && e.name ? e.name : "") + ": " + (e && e.message ? e.message : String(e)) + location + "\\n" + excerpt;');
    lines.push('  } catch(_) { return label + " エラー詳細取得失敗"; }');
    lines.push('}');
...
if (process.env.VJA_TEST_MODE === "1") {
    const testMethods = [
        ...
        "testTblAiGenerateSchema", "testValidAiGenerateRules", "testExtRtGenDoc", "testTextToYamlGenerate", "testManualRetryAiFix", "testFormDesignTextToYamlGenerate", "testYamlAiGenerate", "testSetAutoConfirm", "testGetLastPrompt", "testYamlAiGenerateFull", "testVerifyPromptIntegrity", "testFormDesignAiGenerate",
    ] as const;
