...
const pending = {
    openFile: null as Pending<{
        content: string | null;
        path: string | null;
    }> | null,
    saveFile: null as Pending<{
        ok: boolean;
        path: string | null;
        cancelled: boolean;
    }> | null,
    dbQuery: null as Pending<{
        ok: boolean;
        rows: DbRow[];
        error?: string;
    }> | null,
    dbExecute: null as Pending<{
        ok: boolean;
        result: DbResult;
        error?: string;
    }> | null,
    ...
    dirList: null as Pending<{
        ok: boolean;
        entries: string[];
        error?: string;
    }> | null,
    dirExists: null as Pending<{
        ok: boolean;
        value: boolean;
        error?: string;
    }> | null,
...
w.bunSaveProject = (a: any) =>
    mkPromise("saveFile", () => s.saveFileRequest(a));
...
w.vja = {
    db: {
        ...
        transaction: (statements: { sql: string; params?: any[] }[]) =>
            mkPromise("dbTransaction", () =>
                s.dbTransactionRequest({ statements }),
            ),
    ...
    file: {
        ...
        writeBytes: (path: string, data: number[]) =>
            mkPromise("fileWriteBytes", () =>
                s.fileWriteBytesRequest({ path, data }),
            ),
        exists: (path: string) =>
    ...
    log: {
        info: (message: string) =>
            mkPromise("log", () => s.logRequest({ level: "info", message })),
        warn: (message: string) =>
            mkPromise("log", () => s.logRequest({ level: "warn", message })),
        error: (message: string) =>
            mkPromise("log", () => s.logRequest({ level: "error", message })),
    },
    app: {
        getInfo: () => mkPromise("appInfo", () => s.appInfoRequest({})),
        showDialog: (message: string) =>
            mkPromise("appDialog", () =>
                s.appDialogRequest({ type: "alert", message }),
            ),
        showConfirm: (message: string) =>
            mkPromise("appDialog", () =>
                s.appDialogRequest({ type: "confirm", message }),
            ),
    },
};
