...
`.trim() + "\n\n\n\n\n\n\n\n\n\n\n";
    ...
    const VJA_USE_FRONT_JS_INFO_ENG_ORIGIN = `
...
- Function name: await vja.util.copyToClipboard(text):
- Description: Copies text to the clipboard
- Arguments:
  - text: string - Text to copy
- Return value: boolean - true on successful copy
...
- Function Name: vja.ui.loading(show, message?):
  - Description: Shows/hides the loading overlay
  - Arguments:
    - show: boolean - Show for true, hide for false
    - message?: string - Message to display (default: "Processing...")
  - Return Value: None

## Encryption (vja.crypto.*)

- Function Name: await vja.crypto.encrypt(text, key):
  - Description: Encrypts text using AES-GCM and returns a Base64 string.
  - Arguments:
    - text: string - Text to encrypt
    - key: string - Encryption key (up to 32 characters)
  - Return value: string - Encrypted string in Base64 format

- Function name: await vja.crypto.decrypt(b64, key):
  - Description: Decrypts a Base64 encrypted string.
  - Arguments:
    - b64: string - Encrypted string in Base64 format
    - key: string - Decryption key (same key as used for encryption)
  - Return value: string - Decrypted text
  - Exception: Throws an error if the keys are different.

## Cloud Infrastructure (vja.getCloudInfraCredential)

- Function Name: await vja.getCloudInfraCredential(infra, service?):
  - Description: |
    Retrieves cloud infrastructure credentials (authentication information).
    The credentials defined on the VJA side take precedence, and the key for appInput=ON is obtained from the application's input file (~/vja/credential.json, etc.).
  - Arguments:
    - infra: string - Infrastructure name (e.g., 'AWS', 'GCP (Firebase)', 'Azure (Standard)')
